...
impl fmt::Display for LispExpr {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let res = match self {
            ...
            LispExpr::Boolean(bool) => bool.to_string(),
        };
        write!(f, "{}", res)
    }
}
...
impl LispEnv {
    ...
    fn new() -> Self {
        ...
        // builtins.insert(
        //     "def".to_string(),
        //     LispExpr::Function(|&self, args: &[LispExpr]| -> Result<LispExpr, LispErr> {
        //         if let Some(name, rest) = args.split_first();
        //             if let (parameters, definition) = rest.split_first();
        //                 self.user_defined.insert(
        //                     name.to_string(),
        //                     LispExpr::Function(
        //                         |parameters, definition| -> Result<LispExpr, LispErr> {
        //                             // OK, this is suddenly WAY more complicated than I thought
        //                             // need to insert parameters in place in the definition...
        //                         }
        //                 ))
        //     }
        // ));
    ...
    fn eval(&self, expr: &LispExpr) -> Result<LispExpr, LispErr> {
        ...
        match expr {
            ...
            // Boolean
            LispExpr::Boolean(_) => Ok(expr.clone()),
            // List
            LispExpr::List(list) => {
...
fn parse_atom(token: &str) -> LispExpr {
    ...
    if let Ok(float) = token.parse::<f64>() {
            LispExpr::Number(float)
    } else if let Ok(bool) = token.parse::<bool>() {
            LispExpr::Boolean(bool)
    } else {
        LispExpr::Symbol(String::from(token))
    }
}
